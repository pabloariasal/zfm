...
def invoke_zfm(command, bookmarks_file):
    repo_root = git.Repo('.', search_parent_directories=True).working_tree_dir
    r = subprocess.run(['/bin/zsh', 'run_local_zfm.sh'] + command,
                       cwd=repo_root,
                       capture_output=True,
                       text=True,
...
class Bookmark:
    ...
    def fromLine(cls, bookmark_line):
        ...
        path = Path(s[0].strip())
        type = BookmarkType.Dir if s[1].strip() == '[d]' else BookmarkType.File
...
def list_bookmarks(bookmarks_file):
    output = invoke_zfm(['list'], bookmarks_file)
    return create_bookmarks(output)


def list_file_bookmarks(bookmarks_file):
    output = invoke_zfm(['list', '--files'], bookmarks_file)
    return create_bookmarks(output)


def list_directory_bookmarks(bookmarks_file):
    output = invoke_zfm(['list', '--dirs'], bookmarks_file)
    return create_bookmarks(output)


def test_no_bookmarks(environment):
...
def test_bookmark_single_directory(environment):
    invoke_zfm(['add', environment.dir_a], environment.bookmarks_file)
...
def test_bookmark_single_file(environment):
    invoke_zfm(['add', environment.file_a], environment.bookmarks_file)
    assert list_bookmarks(environment.bookmarks_file) == [
...
def test_bookmark_multiple(environment):
    invoke_zfm(['add', environment.dir_a, environment.file_a],
               environment.bookmarks_file)
    invoke_zfm(['add', environment.dir_b], environment.bookmarks_file)
    assert list_bookmarks(environment.bookmarks_file) == [
        Bookmark(environment.dir_a, BookmarkType.Dir),
        Bookmark(environment.file_a, BookmarkType.File),
...
def test_query(environment):
    invoke_zfm(['add', environment.dir_a, environment.dir_b],
               environment.bookmarks_file)
    invoke_zfm(['add', environment.file_a], environment.bookmarks_file)
    output = invoke_zfm(['query', 'dir_a'], environment.bookmarks_file)
    assert Path(output.strip()) == environment.dir_a
    output = invoke_zfm(['query', '--dirs', 'dir_a'],
                        environment.bookmarks_file)
    assert Path(output.strip()) == environment.dir_a
    output = invoke_zfm(['query', 'file_'], environment.bookmarks_file)
    assert Path(output.strip()) == environment.file_a
    output = invoke_zfm(['query', '--files', 'dir_b/file_a'],
                        environment.bookmarks_file)
    assert Path(output.strip()) == environment.file_a
...
def test_fix(environment):
    ...
    output = invoke_zfm(['fix'], environment.bookmarks_file)
    assert list_bookmarks(environment.bookmarks_file) == []
    assert 'removed 1 entries' in output
...
def test_clear(environment):
    ...
    invoke_zfm(['clear'], environment.bookmarks_file)
    assert list_bookmarks(environment.bookmarks_file) == []
